...
// 1. Struct Declaration: Holds the repository instance
type userService struct {
	userRepo repo.UserRepository
}

// Constructor: Creates a new instance of userService
func NewUserService(repo repo.UserRepository) service.UserService {
	return &userService{userRepo: repo}
}

// 3. Method Implementations:
// Each method implements the UserService interface
// and contains the business logic for user operations

// Register registers a new user
func (s *userService) RegisterNewUser(ctx context.Context, newUserReq *req.CreateUserRequest, newUserPassReq string) (*res.UserResponse, error) {
	...
	if err != nil {
		// 1. ถ้ามี Error และ "ไม่ใช่" Not Found -> คือ DB พัง
	...
	return res.ToUserResponse(newUserDomain), nil
}

// UpdateUserProfile updates the user's profile information
func (s *userService) UpdateUserInfo(ctx context.Context, userID uint, userUpdateReq *req.UpdateUserRequest) (*domain.User, error) {
	...
	// Update user fields
	updatedUser, err := existingUser.UpdateUserProfile(userDomain)
...
func (s *userService) GetUserProfile(ctx context.Context, id uint) (*res.UserResponse, error) {
	...
	return res.ToUserResponse(user), nil
}
