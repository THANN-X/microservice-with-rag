...
const inboxCollection = "catalog_inbox"

type inboxDocument struct {
	// _id เป็น composite ของ messageID + consumerID เพื่อป้องกัน duplicate key
	ID          string    `bson:"_id"`
...
func (r *inboxRepository) MarkProcessed(ctx context.Context, msg *domain.InboxMessage) error {
	...
	if err != nil {
		// duplicate key → message was already processed concurrently, treat as success
...
// compositeID สร้าง unique key จาก messageID + consumerID
